...
def check_amount(fields, request):
    ...
    for product in fields:
        ...
        for percent in range(len(field_percent)):
            ...
            if float(checking_field.amount) != 0:
        ...
        for l in design.designlabel_set.all():
            label = LabelStorage.objects.filter(label__pk=l.label.id, is_active='active').aggregate(Sum('amount'))[
                'amount__sum']
        ...
        if all(state == True for state in states):
            ...
            worker = WorkerAccount.objects.filter(worker__id=product['worker_id']).filter(
                created_at__month=current_month, created_at__year=current_year, completed=False)
            ...
            if len(worker) > 0:
                ...
                worker.workerworks_cost = float(worker.workerworks_cost) + float(product['amount']) * float(
                    product['cost'])
...
def create_price(design):
    ...
    for immutable in immutable_prices:
        if immutable.calc_type == "sum" and immutable.task != "anothers":
            immutable_price += float(immutable.cost)

        if (
                immutable.calc_type == "sum"
                and immutable.task == "anothers"
        ):
    ...
    invalid = base_total + round(base_total / 100 * invalid, 5) if invalid != 0 else base_total

    building_amortization = invalid + round(base_total / 100 * building_amortization, 5)

    machine_amortization = building_amortization + round(base_total / 100 * machine_amortization, 5)

    immutable_percent = machine_amortization + (immutable_percent * 1.24)

    material_weight = round(material_weight, 2)
    material_price_usd = round(material_price_usd, 3)
    base_total = int(round(base_total, 0))
    building_amortization = int(round(building_amortization, 0))
    machine_amortization = int(round(machine_amortization, 0))
    invalid = int(round(invalid, 0))
    immutable_percent = int(round(immutable_percent, 0))
    ...
    design.total = immutable_percent
