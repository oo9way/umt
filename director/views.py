...
from director.utils import get_material_storage_data, get_spare_storage_data, get_label_storage_data, \
    get_pre_production_data, get_product_data, get_sales_data, get_credits_data
from materials.models import Design, Expenditure, Finance, LabelStorage, MaterialStorage, Product, ProductSalesCard, \
    ProductSalesHistory, ProductStock, SpareStorage, MaterialType
...
class MonitorPageView(IsDirectorRole, TemplateView):
    template_name = 'director/monitor.html'

    def get_context_data(self, **kwargs):
        context_data = super().get_context_data(**kwargs)
        context_data["material"] = get_material_storage_data()
        context_data["spare"] = get_spare_storage_data()
        context_data["label"] = get_label_storage_data()
        context_data["pre_production"] = get_pre_production_data()
        context_data['product'] = get_product_data()
        context_data['sales'] = get_sales_data()
        context_data['finance'] = get_credits_data()
...
class HomePageView(IsDirectorRole, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.order_by("-id").filter(is_active="active", amount__gt=0).filter(~Q(amount="0.0"))
        return queryset
